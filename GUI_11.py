...
def method8_multiscale_selective_registration(mask_frame, live_frame, config=None):
    """
    METHOD #8: Hybrid Multi-Scale + Selective Transfer Registration
    
    Complete 6-level pipeline:
        Level 1: Phase Correlation (coarse translation)
        Level 2: ECC Registration (fine + rotation + scale)
        Level 3: Apply Global Transform
        Level 4: Motion Detection (multi-pass)
        Level 5: Optical Flow Refinement (local)
        Level 6: Selective Pixel Transfer
    
    Returns: (registered_frame, metrics)
    """
    if config is None:
    ...
    try:
        logger.info("=" * 60)
        logger.info("METHOD #8: Multi-Scale Selective Registration")
        logger.info("=" * 60)
        
        # Convert to float
        if mask_frame.dtype == np.uint16:
            mask_float = to_float(mask_frame, config.BIT_DEPTH)
        else:
            mask_float = mask_frame.astype(np.float32)
        
        if live_frame.dtype == np.uint16:
            live_float = to_float(live_frame, config.BIT_DEPTH)
        else:
            live_float = live_frame.astype(np.float32)
        
        # LEVEL 1: Phase Correlation
        logger.info(f"LEVEL 1: Phase Correlation ({config.IMAGE_WIDTH}×{config.IMAGE_HEIGHT})...")
        mask_uint16 = from_float(mask_float, config.BIT_DEPTH)
        live_uint16 = from_float(live_float, config.BIT_DEPTH)
        
        dy_pcc, dx_pcc, confidence = phase_correlation_registration(mask_uint16, live_uint16)
        logger.info(f"  → dx={dx_pcc:.3f}, dy={dy_pcc:.3f}, confidence={confidence:.4f}")
        
        # LEVEL 2: ECC Registration
        logger.info(f"LEVEL 2: ECC ({len(config.ECC_PYRAMID_LEVELS)}-level pyramid)...")
        motion_type = cv2.MOTION_EUCLIDEAN if config.ECC_MOTION_TYPE == 'euclidean' else cv2.MOTION_AFFINE
        
        M_ecc, ecc_score, method = ecc_registration(
            mask_float,
            live_float,
            levels=config.ECC_PYRAMID_LEVELS,
        ...
        dx, dy, rotation, scale = calculate_similarity_transform_params(M_ecc)
        logger.info(f"  → dx={dx:.3f}, dy={dy:.3f}, rot={rotation:.3f}°, scale={scale:.5f}")
        logger.info(f"  → ECC Score: {ecc_score:.4f}")
        
        # LEVEL 3: Apply Global Transform
        logger.info("LEVEL 3: Applying global transform...")
        h, w = mask_float.shape
        live_warped = cv2.warpAffine(
            live_float,
            M_ecc,
            (w, h),
        ...
        # LEVEL 4: Motion Detection
        logger.info("LEVEL 4: Multi-pass motion detection...")
        motion_mask, motion_mag = detect_motion_multipass(mask_float, live_warped, config)
        
        num_moving = int(np.sum(motion_mask))
        motion_pct = 100.0 * num_moving / config.TOTAL_PIXELS
        logger.info(f"  → Motion: {num_moving:,}/{config.TOTAL_PIXELS:,} ({motion_pct:.2f}%)")
        
        # LEVEL 5: Optical Flow Refinement
        if config.ENABLE_LOCAL_REFINEMENT and motion_pct > 0.5:
            logger.info("LEVEL 5: Optical flow refinement...")
            live_refined = refine_with_optical_flow(mask_float, live_warped, motion_mask, config)
            logger.info("  → Refinement applied")
        else:
            live_refined = live_warped
            if motion_pct <= 0.5:
                logger.info("LEVEL 5: Skipped (motion < 0.5%)")
        
        # LEVEL 6: Selective Pixel Transfer
        logger.info("LEVEL 6: Selective pixel transfer...")
        registered = selective_pixel_transfer(
            mask_float,
            live_float,
            live_refined,
            motion_mask,
        ...
        logger.info("=" * 60)
        logger.info("✅ METHOD #8 COMPLETE")
        logger.info("=" * 60)
        
        # Metrics
        metrics = {
            'method': 'method8_multiscale_selective',
            'dx': float(dx),
            'dy': float(dy),
            'rotation': float(rotation),
            'scale': float(scale),
            'pcc_confidence': float(confidence),
            'ecc_score': float(ecc_score),
            'motion_pixels': num_moving,
        ...
        logger.exception("Method #8 failed")
        if live_frame.dtype == np.uint16:
            return to_float(live_frame), {'method': 'method8_multiscale_selective', 'error': str(e)}
        else:
            return live_frame, {'method': 'method8_multiscale_selective', 'error': str(e)}
...
def bias_field_correction(dsa, strength=0.003, radius=55):
    """Low-frequency background flattening to remove residual shading."""
    background = cv2.GaussianBlur(dsa, (0, 0), radius)
    corrected = dsa - strength * background
        ...
        try:
            ...
            for i, contrast_frame in enumerate(frames_registered):
                if i == mask_idx:
                    ...
                    dsa = perform_dsa(mask_frame, contrast_frame)
                    dsa = bias_field_correction(dsa)
...
class DicomViewer(QWidget):
    ...
    def process_dsa_pipeline(self):
        ...
        params = {
            ...
            'wiener_kernel': 5,
            'WL': self.wl_slider.value()/40.0,
            'WW': self.ww_slider.value()/40.0,
