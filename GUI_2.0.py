...
def apply_smooth_grid_transforms(img_float, transforms):
    """
    Apply grid transformations with smooth blending between cells
    Uses weighted interpolation based on distance to grid centers
    """
    h, w = img_float.shape
    ...
    for trans_dict in transforms:
        ...
        x1, y1, x2, y2 = trans_dict['bbox']
        
        # Create weight map (inverse distance from center)
        weight_map = np.zeros((h, w), dtype=np.float32)
        
        # Gaussian-like weights centered on this grid cell
        for y in range(y1, y2):
            for x in range(x1, x2):
                dist = np.sqrt((x - cx)**2 + (y - cy)**2)
                max_dist = np.sqrt((x2 - x1)**2 + (y2 - y1)**2) / 2
                weight_map[y, x] = np.exp(-2 * (dist / max_dist)**2)
...
def estimate_grid_transforms(src_img, dst_img, grid_size=(4, 4), method='ecc'):
    """
    Divide image into grid and compute transformation for each cell
    ...
    Returns:
        transforms: List of (transform_matrix, bbox) tuples
    """
    h, w = src_img.shape
    ...
    for i in range(grid_h):
        for j in range(grid_w):
            ...
            if src_patch.std() < 0.01 or dst_patch.std() < 0.01:
                # Low contrast patch - use identity transform
                transform = np.array([[1, 0, 0], [0, 1, 0]], dtype=np.float32)
            else:
                # Compute transformation for this patch
                transform = estimate_affine_transform(src_patch, dst_patch, method=method)
...
def apply_grid_transforms(img_float, transforms, interpolation='linear'):
    """
    Apply grid-based transformations
    
    Args:
        img_float: Input image
        transforms: List of transform dictionaries from estimate_grid_transforms
        interpolation: 'linear' or 'smooth' (with blending)
    ...
    if interpolation == 'linear':
        # Simple approach: transform each grid cell independently
        for trans_dict in transforms:
            transform = trans_dict['transform']
            x1, y1, x2, y2 = trans_dict['bbox']
            ...
            transformed_patch = cv2.warpAffine(
                patch, transform, (patch_w, patch_h),
                flags=cv2.INTER_LINEAR,
                borderMode=cv2.BORDER_REFLECT
            )
            
            result[y1:y2, x1:x2] = transformed_patch
    
    else:  # 'smooth' - with blending
        result = apply_smooth_grid_transforms(img_float, transforms)
...
def estimate_tps_transform(src_img, dst_img, grid_size=(4, 4)):
    """
    Use grid transforms to create control points for Thin-Plate Spline warping
    This creates smooth, continuous deformation field
    """
    from scipy.interpolate import Rbf
    
    # Get grid transforms
    transforms = estimate_grid_transforms(src_img, dst_img, grid_size)
    
    # Extract control points and their displacements
    src_points = []
    dst_points = []
    ...
    for trans_dict in transforms:
        ...
        dst_points.append([transformed[0], transformed[1]])
    
    src_points = np.array(src_points)
    dst_points = np.array(dst_points)
    
    # Create displacement field interpolators
    dx = dst_points[:, 0] - src_points[:, 0]
    dy = dst_points[:, 1] - src_points[:, 1]
    
    rbf_x = Rbf(src_points[:, 0], src_points[:, 1], dx, function='thin_plate', smooth=0.1)
    rbf_y = Rbf(src_points[:, 0], src_points[:, 1], dy, function='thin_plate', smooth=0.1)
    
    return rbf_x, rbf_y

def apply_tps_transform(img_float, rbf_x, rbf_y):
    ...
    dx = rbf_x(x.ravel(), y.ravel()).reshape((h, w))
    dy = rbf_y(x.ravel(), y.ravel()).reshape((h, w))
    ...
    # Remap image
    result = cv2.remap(img_float, map_x, map_y, cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT)
...
def estimate_affine_transform(src_img, dst_img, method='orb'):
    ...
    if use_grid and method == 'ecc':
        # Grid-based registration
        return estimate_grid_transforms(src_img, dst_img, grid_size, method='ecc')
    if method == 'orb':
        ...
        criteria = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 3000, 1e-10)
...
def apply_affine_transform(img_float, transform_matrix, use_inverse=True):
    """Apply affine transformation to image using ECC or ORB matrix."""
    h, w = img_float.shape
    transform_matrix = transform_matrix.astype(np.float32)
    
    # ECC finds transform that maps dst → src,
    # but we want to map src → dst (inverse direction)
    flags = cv2.INTER_LINEAR
    if use_inverse:
