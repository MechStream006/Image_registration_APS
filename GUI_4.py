...
# Try to import post-processing helpers from Post_processing.py (robust)
try:
    from Post_processing import (
        gamma_correction as pp_gamma_correction,
        gaussian_denoise as pp_gaussian_denoise,
        nlm_denoise as pp_nlm_denoise,
        clahe_enhance as pp_clahe_enhance,
        unsharp_mask as pp_unsharp_mask
    )
except Exception:
    # Fallback: ensure current directory is on path and try again
    import sys
    sys.path.append(os.path.dirname(__file__) if '__file__' in globals() else os.getcwd())
    from Post_processing import (
        gamma_correction as pp_gamma_correction,
        gaussian_denoise as pp_gaussian_denoise,
        nlm_denoise as pp_nlm_denoise,
...
# =========================================================
# Image Processing Functions (existing functions from GUI_2)
# =========================================================
...
# =========================================================
# Processing Thread (with integrated Post-Processing via Post_processing.py)
# =========================================================

class DSAProcessingThread(QThread):
    """Background thread for DSA processing + automatic post-processing"""
    ...
    def run(self):
        try:
            ...
            # Step 4: Perform DSA & Step 5: Automatic Post-processing (using Post_processing.py helpers)
            ...
            for i, contrast_frame in enumerate(frames_registered):
                ...
                # Post-processing pipeline (always run)
                # Use functions imported from Post_processing.py (pp_*)
                try:
                    img = pp_gamma_correction(dsa_result, gamma=0.5)
                    img = pp_gaussian_denoise(img, sigma=0.6)
                    img = pp_nlm_denoise(img, h=0.0125)
                    img = pp_clahe_enhance(img, clip_limit=2, tile_grid_size=(4,4))
                    img = pp_unsharp_mask(img, amount=0.45, radius=0.35)
                except Exception:
                    # If post-processing functions fail for any reason, fallback to a light pipeline
                    img = gamma_correction(dsa_result, gamma=0.5)
                    img = wiener_filter(img, kernel_size=3)
                    img = apply_clahe(img, clip_limit=2, tile_grid_size=(4,4))
                    img = apply_unsharp_mask(img)
                
                frames_dsa_post.append(img)
                
                progress_pct = 85 + int(15 * (i + 1) / len(self.frames))
                self.progress.emit(progress_pct, f"Post-processing frame {i+1}/{len(self.frames)}")
            
            # Use post-processed DSA frames as the frames to return/display
            frames_dsa = frames_dsa_post
            
            self.progress.emit(100, "Processing + Post-processing complete!")
