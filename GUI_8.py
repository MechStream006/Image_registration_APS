# GUI_7_improved.py
# Improved version of GUI_7.py with additional robustness, logging,
# registration metrics export, ECC retry/fallback and safer memory handling.
...
# =========================================================
# GPU-Accelerated Preprocessing Functions (same as before)
# =========================================================
...
# =========================================================
# Histogram Matching (same as before)
# =========================================================
...
# =========================================================
# Phase Correlation Registration (same core but returns confidence)
# =========================================================
...
def phase_correlation_registration(mask_frame, live_frame):
    ...
    Returns (dy, dx, peak_value) - translation shifts and peak correlation value as confidence.
    """
    try:
        if GPU_AVAILABLE:
...
# =========================================================
# ECC Registration with retry, fallback to affine and optical flow
# =========================================================

def _extract_rotation_and_scale_from_matrix(M):
    # M is 2x3 affine (a b tx; c d ty)
    a, b, tx = M[0]
    c, d, ty = M[1]
    # Rotation angle
    angle = np.degrees(np.arctan2(c, a))
    # Scales = length of column vectors
...
def ecc_registration(mask_frame, live_frame, levels=(0.25, 0.5, 1.0),
    ...
    """
    Multi-scale ECC registration with retries and fallback.
    ...
    # Try progressively more flexible transforms on failure
    ...
    for attempt in attempts:
        ...
        try:
            ...
            # if last attempt, continue to next
            continue

    # If ECC did not give a meaningful score, fallback to optical flow warp
    if ecc_score <= 0.2:
        try:
            logger.info("ECC score low, attempting optical flow fallback")
            # compute dense flow on uint8
            ...
            # Build near-identity affine (we'll return warped image in a wrapper if needed)
            warp_matrix = np.eye(2,3, dtype=np.float32)
            method_used = 'optical_flow'
            ecc_score = 0.0
            # Store warped in a small temporary place - function returns matrix, callers should check method_used
            # We'll attach warped image as attribute (non-ideal but keeps API stable)
...
def apply_affine_warp(img_float, transform_matrix):
    ...
    # If transform_matrix carries a precomputed warped image (optical flow fallback), use it
    if hasattr(transform_matrix, '_warped_image'):
        warped = transform_matrix._warped_image
        return np.clip(warped, 0, 1)
    registered = cv2.warpAffine(img_float, transform_matrix, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT)
    return np.clip(registered, 0, 1)

# =========================================================
# Hybrid registration (updated to collect metrics)
# =========================================================
...
def hybrid_registration(mask_frame, live_frame):
    ...
    # If ECC returned a pre-warped image via optical flow, we should not add translations to matrix
...
# =========================================================
# DSA Subtraction and Post-process (unchanged)
# =========================================================
...
def post_process_dsa(dsa_frame, gamma=0.5, gaussian_sigma=0.6, 
    ...
    if kernel_size % 2 == 0:
        kernel_size += 1
    img = apply_unsharp_mask_gpu(img, kernel_size=(kernel_size, kernel_size), sigma=unsharp_radius, amount=unsharp_amount)
    return np.clip(img, 0, 1)

# =========================================================
# Processing Thread - improved: logs, csv dump, memory handling
# =========================================================
        ...
        try:
            ...
            for i, frame in enumerate(frames_preprocessed):
                if i == mask_idx:
                    ...
                    self.registration_log.append({'frame': i, 'method': 'mask', 'dx': 0.0, 'dy': 0.0, 'angle': 0.0, 'ecc_score': 1.0})
                else:
                    ...
                    if registration_method == 'pcc':
                        ...
                        log_entry = {'frame': i, 'method': 'pcc', 'dx': dx, 'dy': dy, 'angle': 0.0, 'ecc_score': peak}

                    elif registration_method == 'hybrid':
                        # Step 1 – PCC translation + Step 2 ECC refinement
                        ...
                        log_entry = {'frame': i, 'method': metrics.get('method', 'hybrid'),
                                     ...
                                     'angle': metrics.get('angle', 0.0), 'ecc_score': metrics.get('ecc_score', 0.0)}
                        ...
                        registered = apply_affine_warp(frame, M)
                        angle, sx, sy = _extract_rotation_and_scale_from_matrix(M)
                        # If method_used == 'optical_flow', M may carry _warped_image used by apply_affine_warp
                        log_entry = {'frame': i, 'method': method_used, 'dx': M[0,2], 'dy': M[1,2], 'angle': angle, 'ecc_score': ecc_score}
            ...
            for i, contrast_frame in enumerate(frames_registered):
                if i == mask_idx:
                    ...
                    dsa_result = perform_dsa(mask_frame, contrast_frame)
...
# =========================================================
# DICOM Save Utilities (unchanged)
# =========================================================
...
def save_multiframe_dicom(original_dcm, frames_float, out_path, description_suffix=""):
    ...
    logger.info(f"✅ Saved multi-frame DICOM (16-bit): {out_path}")
...
def save_single_frame_dicom(original_dcm, frame_float, out_path, frame_number=0, description_suffix=""):
    ...
    new_dcm.save_as(out_path, write_like_original=False)
    logger.info(f"✅ Saved single-frame DICOM (16-bit): {out_path}")

# =========================================================
# DICOM Viewer Widget and Main Window
# (kept as close to original structure as possible; minimal UI changes)
# =========================================================
...
class DicomViewer(QWidget):
    ...
    def setup_dsa_controls(self, layout):
        ...
        self.reg_combo.addItems(['PCC (Phase Correlation)', 'ECC (Enhanced Correlation)', 'Hybrid (PCC + ECC)', 'Local Flow (Farneback GPU)',  'Local Flow (TV-L1 GPU)'])
    ...
    def save_current_frame_dicom(self):
        if self.frames_dsa is None or self.original_dicom is None:
            ...
            return
        filename, _ = QFileDialog.getSaveFileName(self, "Save Current Frame as DICOM", f"dsa_frame_{self.frame_index:03d}.dcm", "DICOM Files (*.dcm)")
        if filename:
            try:
                frame = self.frames_dsa[self.frame_index]
                save_single_frame_dicom(self.original_dicom, frame, filename, frame_number=self.frame_index, description_suffix="DSA Result")
    ...
    def save_multiframe_dicom_dsa(self):
        if self.frames_dsa is None or self.original_dicom is None:
            QMessageBox.warning(self, "No Data", "Please process DSA first!")
            return
        filename, _ = QFileDialog.getSaveFileName(self, "Save DSA as Multi-frame DICOM", "dsa_results_multiframe.dcm", "DICOM Files (*.dcm)")
        if filename:
            try:
                save_multiframe_dicom(self.original_dicom, self.frames_dsa, filename, description_suffix="DSA Results")
                QMessageBox.information(self, "Success", f"Saved {len(self.frames_dsa)} frames to:\n{filename}")
            except Exception as e:
    ...
    def save_multiframe_dicom_preprocessed(self):
        if self.frames_preprocessed is None or self.original_dicom is None:
            ...
            return
        filename, _ = QFileDialog.getSaveFileName(self, "Save Preprocessed as Multi-frame DICOM", "preprocessed_multiframe.dcm", "DICOM Files (*.dcm)")
        if filename:
            try:
                save_multiframe_dicom(self.original_dicom, self.frames_preprocessed, filename, description_suffix="Preprocessed")
                QMessageBox.information(self, "Success", f"Saved {len(self.frames_preprocessed)} frames to:\n{filename}")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to save DICOM:\n{str(e)}")
    ...
    def save_multiframe_dicom_registered(self):
        if self.frames_registered is None or self.original_dicom is None:
            ...
            return
        filename, _ = QFileDialog.getSaveFileName(self, "Save Registered as Multi-frame DICOM", "registered_multiframe.dcm", "DICOM Files (*.dcm)")
        if filename:
            try:
                save_multiframe_dicom(self.original_dicom, self.frames_registered, filename, description_suffix="Registered")
                QMessageBox.information(self, "Success", f"Saved {len(self.frames_registered)} frames to:\n{filename}")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Failed to save DICOM:\n{str(e)}")
    ...
    def on_processing_finished(self, preprocessed, registered, dsa):
        # preprocessed list is intentionally empty to save memory
        self.frames_preprocessed = None
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Advanced DSA Viewer - GPU-Accelerated Pipeline (Improved)")
    ...
    def open_dicom(self):
        filename, _ = QFileDialog.getOpenFileName(self, "Open DICOM File", "", "DICOM Files (*.dcm);;All Files (*)")
        if filename:
...
if __name__ == "__main__":
    ...
    if GPU_AVAILABLE:
        logger.info("=" * 70)
        logger.info("GPU-ACCELERATED DSA PIPELINE (Improved)")
        logger.info("=" * 70)
        get_gpu_memory_info()
    else:
        logger.info("=" * 70)
        logger.info("DSA PIPELINE - CPU MODE (Improved)")
