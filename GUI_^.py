...
def ecc_registration(mask_frame, live_frame, 
                     ...
                     verbose=False):
    """
    Enhanced ECC (Enhanced Correlation Coefficient) registration with normalization and pyramids.

    Args:
        mask_frame (np.ndarray): Reference image (float32, range [0,1])
        live_frame (np.ndarray): Frame to align (float32, range [0,1])
        motion_model (int): ECC motion model (cv2.MOTION_TRANSLATION, cv2.MOTION_EUCLIDEAN, cv2.MOTION_AFFINE)
        num_levels (int): Number of pyramid levels (coarse-to-fine)
        max_iterations (int): Maximum iterations at each level
        epsilon (float): ECC termination epsilon
        verbose (bool): Print debug info (optional)

    Returns:
        warp_matrix (np.ndarray): 2x3 affine/translation transform matrix
        cc_value (float): final ECC correlation coefficient
    """
    # --- Convert to float32 and normalize intensity ---
    ref = mask_frame.astype(np.float32)
    mov = live_frame.astype(np.float32)

    # Normalize each to mean 0, std 1 to improve ECC convergence
    ...
    try:
        # --- Multiscale pyramid ECC registration ---
...
# =========================================================
# Local (Grid-Based) ECC Registration
# =========================================================

def divide_into_tiles(img, grid_size=(8, 8)):
    """Divide image into grid tiles (returns list of (y0, y1, x0, x1))."""
    h, w = img.shape
    gh, gw = grid_size
...
def local_ecc_registration(mask_frame, live_frame, grid_size=(8, 8)):
    """
    Perform local (piecewise) ECC registration using translation model.
    Returns dense motion field (flow) of shape (H, W, 2).
    """
    ...
    for (y0, y1, x0, x1) in tiles:
        ...
        # Skip uniform tiles (no texture)
        if np.std(ref_tile) < 1e-5 or np.std(mov_tile) < 1e-5:
            continue

        tile_h, tile_w = ref_tile.shape
        if tile_h < 30 or tile_w < 30:
            continue
        if np.std(ref_tile) < 1e-5 or np.std(mov_tile) < 1e-5:
            continue
    ...
    # Smooth motion field across tile boundaries
...
def warp_with_flow(img, flow):
    """
    Apply nonrigid warp based on dense flow field (dx, dy per pixel).
    """
    h, w = img.shape
    ...
    if GPU_AVAILABLE:
        try:
            ...
            coords = cp.array([map_y_gpu, map_x_gpu])
            warped = map_coordinates(img_gpu, coords, order=1, mode='reflect')
            return cp.asnumpy(cp.clip(warped, 0, 1))
        except Exception as e:
            print(f"⚠️ GPU warp failed, falling back to CPU: {e}")

    warped = cv2.remap(img.astype(np.float32), map_x, map_y, cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT)
    return np.clip(warped, 0, 1)
        ...
        try:
            ...
            for i, frame in enumerate(frames_preprocessed):
                if i == mask_idx:
                    ...
                    elif registration_method == 'ecc_local':
    # Grid-based local ECC registration
                        self.progress.emit(55, "Performing local ECC registration...")
                        motion_field = local_ecc_registration(mask_frame, frame, grid_size=(8, 8))
                        registered = warp_with_flow(frame, motion_field)
                    else:  # ecc
                        motion_model = {
                        ...
                        motion_model = motion_model.get(self.params['registration_method'], cv2.MOTION_AFFINE)
                        
                        transform, cc_val = ecc_registration(mask_frame, frame, motion_model=motion_model)
                        if cc_val < 0.5:
...
# Image display
    ...
    def setup_dsa_controls(self, layout):
        ...
        self.reg_combo.addItems(['PCC (Phase Correlation)', 'ECC (Affine)', 'ECC (Euclidean)', 'ECC (Translation)', 'ECC (Local Grid)'])
    ...
    def process_dsa_pipeline(self):
        ...
        # Determine registration method from combo box
